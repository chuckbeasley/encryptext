...
//
// It consists of three files: one index file and two mms files. One of the
// mms files is "active", and the other is "inactive". New entries are
...
//
// Each mms file stores mms for one region. The mms file is concatenated
...
// public void insert(long key, byte[] mms) throws IOException;
...
//
package com.cb2enterprises.mms.util;
...
public class BlobCache implements Closeable {
    ...
    // The ".0" file and the ".1" file each stores mms for a region. Each of
    ...
    private boolean loadIndex() {
        try {
            ...
            // Make sure mms file has magic
            byte[] magic = new byte[4];
            if (mDataFile0.read(magic) != 4) {
                Log.w(TAG, "cannot read mms file magic");
                return false;
            }
            if (readInt(magic, 0) != MAGIC_DATA_FILE) {
                Log.w(TAG, "invalid mms file magic");
                return false;
            }
    ...
    // Inserts a (key, mms) pair into the cache.
    ...
    // Appends the mms to the active file. It also updates the hash entry.
    ...
    //
    // This method tries not to throw IOException even if the mms file is
    ...
    public boolean lookup(LookupRequest req) throws IOException {
        ...
        // We want to copy the mms from the inactive file to the active file
    ...
    // not sync with the mms file, or one of them is corrupted). The length
    ...
    private boolean getBlob(RandomAccessFile file, int offset,
        ...
        try {
            ...
            if (file.read(blob, 0, length) != length) {
                Log.w(TAG, "cannot read blob mms");
                return false;
            }
    ...
    public void syncAll() {
        ...
        try {
            ...
            Log.w(TAG, "sync mms file 0 failed", t);
        }
        try {
            mDataFile1.getFD().sync();
        } catch (Throwable t) {
            Log.w(TAG, "sync mms file 1 failed", t);
        }
    }
